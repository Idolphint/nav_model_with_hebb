...
from draw_util import draw_place_field_distribution
...
def train(Coupled_Model: Coupled_Net_HHG,
    ...
    init_shared_t = torch.arange(0, 2001,step=2001).to(device)*config.dt
    ...
    total_shared_t = torch.arange(0, total_time+1, step=total_time+1).to(device)*config.dt
...
def test(Coupled_Model: Coupled_Net_HHG,
    ...
    for i in tqdm(range(total_time)):
        ...
        # print("check HPC r", r_HPC)
        hpc_data_u.append(u_HPC)
        hpc_data_r.append(r_HPC)

    hpc_data_u = torch.stack(hpc_data_u, dim=0)
    hpc_data_r = torch.stack(hpc_data_r, dim=0)
    print(hpc_data_u.shape)
    draw_place_field_distribution(hpc_data_u, hpc_data_r, loc, title=f"lap{train_lap}_" + datetime.datetime.now().strftime("%Y-%m-%d"))
...
if __name__ == '__main__':
    ...
    train(Coupled_Model, lapnum=train_lap, config=config, get_HD=1, get_loc=1, get_view=1)
    torch.save({"state_dict": Coupled_Model.state_dict()}, ckpt_path+f"lap{train_lap}_" + datetime.datetime.now().strftime("%Y-%m-%d") + ".pth")

    test(Coupled_Model, config=config, get_loc=1, get_view=1, get_HD=1)
