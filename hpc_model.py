...
class HPCModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.u = torch.zeros([self.num, ]).to(device)
        ...
        # Conn matrix
        self.W_sen_back = torch.zeros([self.num_sen, self.num]).to(device)
        self.W_E2E = torch.zeros([self.num, self.num]).to(device)
        self.corr = torch.ones((self.num, self.num_sen)).to(device)
        self.corr = torch.ones((self.num, self.num_sen)).to(device)
        self.W_sen = torch.randn((self.num, self.num_sen)).to(device) * 0.1
        # self.W_sen = randomize_matrix(self.W_sen, percent_zeros=percentage)
        self.W_sen = normalize_rows(self.W_sen) * self.norm_sen_exc
    ...
    def conn_update_forward(self, r_sen):
        ...
        self.W_sen = normalize_rows(W_sen) * self.norm_sen_exc
    ...
    def conn_update_rec(self, thres=3.5):
        ...
        self.W_E2E = torch.where(W_E2E > 0, W_E2E, 0)
    ...
    def conn_update_back(self, r_sen, thres=3.5):
        ...
        corr_mat_sen = torch.outer(r_sen, r_exc)
        ...
        self.W_sen_back = torch.where(W_sen_back > 0, W_sen_back, 0)
