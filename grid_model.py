...
class GridModel(nn.Module):
    ...
    def __init__(self, ratio, angle, config):
        ...
        self.rot = torch.Tensor([[np.cos(self.angle), -np.sin(self.angle)],[np.sin(self.angle), np.cos(self.angle)]]).to(device)

        # initialize conn matrix
        self.conn_mat = self.make_conn()
        self.conn_out = self.make_conn_out()
        self.conn_out = self.norm_fac * normalize_rows(self.conn_out)
        self.conn_in = torch.zeros([self.num, self.num_hpc]).to(device)
    ...
    def conn_out_update(self, r_learn_hpc):
        ...
        self.conn_out = self.norm_fac * normalize_rows(conn_out) * np.exp(-self.ratio**2/5**2)
